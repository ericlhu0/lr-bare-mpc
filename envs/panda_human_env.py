...
import numpy as np
from pybullet_helpers import inverse_kinematics
...
class PandaHumanRepositioningEnv(DynamicsModelEnv):
    ...
    def _get_default_scene_config(
        ...
        panda_init_joint_velocities=[0.0] * 6,
        human_init_joint_velocities=[0.0] * 6,
        ...
        torque_lower_limits=[-1] * 6,
        torque_upper_limits=[1] * 6,
        camera_target=(0.15, 0.1, 1.4),
        camera_distance=1.75,
        camera_pitch=-50,
        image_height=512,
        image_width=512,
    ) -> RepositioningSceneConfig:
